...
Tables~\ref{tab:func_requirements} and~\ref{tab:nonfunc_requirements} list the FRs and NFRs respectively.
...
\hline
\endhead
PR1 & Extrusion speed\textsuperscript{1} & 5 & 1 - 10 & mm/s & FR2 \\
\hline
PR2 & Positional accuracy along XY axes\textsuperscript{2} & 100 & 1 - 500 & $\mu$m
 & FR1 \\
\hline
PR3 & Positional accuracy along Z axis\textsuperscript{2} & 50 & 1 - 100 & $\mu$m
 & FR1 \\
\hline
PR4 & Maximum travel speed along XY axes\textsuperscript{1} & 10 & 0 - 50 & mm/s & FR1 \\
\hline
PR5 & Minimum bed temperature\textsuperscript{3} & 4 & < 20 & $^{\circ}$C & FR4 \\
\hline
PR6 & Maximum nozzle temperature\textsuperscript{4} & 85 & 80 < & $^{\circ}$C & FR4 \\
\hline
PR7 & Nozzle temperature control tolerance & $\pm$ 1 & $\pm$ 0.5 – 2 & $^{\circ}$C & FR4 \\
\hline
PR8 & Extrusion force control tolerance & $\pm$ 5 & $\pm$ 2 – 10 & \% & FR2, FR7 \\
\hline
\end{longtable}
{\footnotesize
\noindent\textsuperscript{1} Recommended print speed of 0-10 mm/s for the BIO X \citep{cellink_biox_manual_2023}.

\noindent\textsuperscript{2} The Bio X has a 1~µm XY/Z positional accuracy \citep{biox_brochure}.

\noindent\textsuperscript{3} This matches the Bio X's minimum bed temperature capability of 4 $^{\circ}C$ \citep{biox_brochure}.

\noindent\textsuperscript{4} The target matches the maximum capability of the Bio X's pneumatic print heads \citep{biox_brochure}.
}
...
\textbf{Rating} & \textbf{Motivation} \\
\hline
Maximum flow rate (SR1) & 0.07 & +2 & Higher throughput & 0 & Limited by lead screw and drive-train & 0 & Limited by lead screw and drive-train\\\hline
Maximum extrusion force (SR1) & 0.13 & -2 & Head losses due to longer fluid path & 0 & High due to drive-train & 0 & High due to drive-train \\\hline
...
Flow rate accuracy (SR3) & 0.19 & -1 & Inaccurate at low speeds & 0 & Accurate due to lead screw and gear reduction & 0 & Accurate due to lead screw and drive-train \\\hline
...
Affordability (SR11) & 0.15 & -2 & Requires a peristaltic pump & 0 & Standard parts & 0 & Standard parts \\\hline
\hline
...
\noindent To convert this to a volumetric resolution, this value must be multiplied by the area of the syringe's cross-section through which the plunger slides:
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{figs/GantryConcept2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{figs/GantryConcept1.png}
...
\noindent This theoretical accuracy outperforms the \SI{50}{\micro\meter} requirement established in PR3. It should be noted that an even higher resolution can be achieved with microstepping.

\subsubsection*{X and Y Axis Stepper Requirements and Positional Accuracy}
Autodesk Inventor Professional can be used to estimate the final extruder's total volume of \SI{379353.531}{\milli\meter^3} and the printer bed with its linear bearings, heat sink, Peltier and cold plate (shown in the final design later in Figure~\ref{fig:gantryFinal}) has a total volume of \SI{145105.084}{\milli\meter^3}. As done previously, by multiplying these volumes by the density of aluminium (\SI{2.7e-6}{\kilogram\per\milli\meter^{3}}), yielding two respective mass approximations of \( m_x = 0.39\,\text{kg} \) and \( m_y = 1.02\,\text{kg}\). By further assuming the carriages' friction coefficient to be 0.2 and the maximum horizontal acceleration to be \( a = 5\,\text{m/s}^2 \), the required stepper torque can be determined for each axis, as shown in Appendix A.3. The X-axis uses a GT2 pulley with 20 teeth and the calculations show that the stepper requires $0.0173~N\cdot m$ of torque and the Y-axis stepper (which uses a 36 teeth pulley) needs $0.08137~N\cdot m$. Therefore, the chosen NEMA 17 steppers have a holding torque up to \SI{0.28}{\newton\meter} for the X-axis and \SI{0.4}{\newton\meter} for the Y-axis.

With these described specifications, the positional accuracy in horizontal plane can be determined. Considering the use of pulleys in the X and Y directions, the resolution can determined by multiplying the motors step angle with the radius of each pulley ($r_x=6.365$ and $r_y=11.46$):
\begin{align*}
...
\noindent
The complete bioprinter design, shown in Figure~\ref{fig:bioprinterCAD}, shows the extruder and gantry subsystems, integrated into a single mechanical system. The extruder is designed to use a custom syringe, heater block, and load cell to deposit viscous gels at a controlled flow rate and temperature, while the gantry accurately positions it in 3D space to enable it to print three dimensional scaffolds. This final design concludes the mechanical design of the project and forms the basis for the embedded systems design and integration to enable the printer to function as required and undergo experimental validation. To provide a more detailed view of the bioprinter's design, the mechanical drawings of all custom parts, as well as the gantry and extruder assembly drawings, are available in the project’s \href{https://github.com/SimonDaniel605/Hydrogel-3D-Printer/tree/main/Hardware/Bioprinter%20Machine%20Design/Final%20System/HydrogelBioprinter-DrawingPack/Ready%20to%20print}{GitHub repository's \texttt{Hardware} folder} \citep{Daniel2025_HydrogelPrinter}.

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.9]{figs/FinalBioprinter.png}
